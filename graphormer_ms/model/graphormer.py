...
class GraphormerModel(FairseqEncoderModel):
    ...
    def construct(self, batched_data, **kwargs):
...
class GraphormerEncoder(FairseqEncoder):
    def __init__(self, args):
        ...
        self.masked_lm_pooler = nn.Dense(
            args.encoder_embed_dim, args.encoder_embed_dim
        )

        self.lm_head_transform_weight = nn.Dense(
            args.encoder_embed_dim, args.encoder_embed_dim
        )
        ...
        if self.load_softmax:
            self.lm_output_learned_bias = ms.Parameter(ops.zeros(1))

            if not self.share_input_output_embed:
                self.embed_out = nn.Dense(
                    args.encoder_embed_dim, args.num_classes, has_bias=False
                )
            else:
    ...
    def construct(self, batched_data, perturb=None, masked_tokens=None, **unused):
        ...
        if self.share_input_output_embed and hasattr(
            ...
            x = ops.mm(x, self.graph_encoder.embed_tokens.weight)
